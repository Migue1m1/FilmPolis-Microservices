...
export class Repository {
    ...
    public getSuggestions  (req: Request, res: Response, next: NextFunction) {
        ...
        Actor.find(query, { _id: 0, name: 1, imageURL: 1 }).limit(10).exec((err, suggestions) => {
            if (err) {
                return res.status(400).json({ message: 'Error during find Actor', status: 400, error: err });
            }
            if (suggestions && suggestions != []) {
                res.status(200).json({ message: 'Actor found successfully', status: 200, data: suggestions });
            }
            else
                res.status(404).json({ message: 'Actor not found', status: 404 });
        });
    }
